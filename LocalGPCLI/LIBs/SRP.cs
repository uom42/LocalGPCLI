...
namespace LGPOSRPCLI
{
	...
	internal class SRP
	{
		...
		public LEVELS Level
		{
			...
			set => Policy_SetSaferValue<int>(C_SAFER_VALUE_NAME_DefaultLevel, (int)value, DeleteRegistryValueIfNullValue: false);
		}

		public SCOPES Scope
		{
			get => Policy_GetSaferValue(C_SAFER_VALUE_NAME_PolicyScope, SCOPES.ALL_USERS);
			set => Policy_SetSaferValue<int>(C_SAFER_VALUE_NAME_PolicyScope, (int)value, DeleteRegistryValueIfNullValue: false);
		}
		...
		public Boolean AuthenticodeEnabled
		{
			...
			set => Policy_SetSaferValue<bool>(C_SAFER_VALUE_NAME_AuthenticodeEnabled, value, DeleteRegistryValueIfNullValue: false);
		}
		...
		public TRANSPARENTS TransparentEnabled
		{
			...
			set => Policy_SetSaferValue<int>(C_SAFER_VALUE_NAME_TransparentEnabled, (int)value, DeleteRegistryValueIfNullValue: false);
		}
		...
		internal void Policy_SetSaferValue<T>(String Name,
			T? oValue,
			RegistryValueKind? Kind = null,
			string SaferKeyPath = C_SAFER_CI_KEY,
			Boolean DeleteRegistryValueIfNullValue = true) where T : unmanaged
		{
			_ = SaferKeyPath ?? throw new ArgumentNullException(nameof(SaferKeyPath));

			Policy_OpenSaferKey_Machine(SaferKeyPath,
				true,
				hKeyCodeIdentifiers => hKeyCodeIdentifiers.e_SetValue<T>(Name, oValue));
		}
			...
			Boolean DeleteRegistryValueIfNullValue = true)
		{
			...
			Policy_OpenSaferKey_Machine(SaferKeyPath,
				true,
				hKeyCodeIdentifiers => hKeyCodeIdentifiers.e_SetValueString(Name, oValue));
		}
			...
			Boolean DeleteRegistryValueIfNullValue = true)
		{
			...
			Policy_OpenSaferKey_Machine(SaferKeyPath,
				true,
				hKeyCodeIdentifiers => hKeyCodeIdentifiers.e_SetValueStrings(Name, oValue));
		}
